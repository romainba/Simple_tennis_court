...
function toascii( $str )
{
    return strtr(utf8_decode($str), 
        utf8_decode('ÂÃÄÅÆÇÈÉÊÍÎÏÐÑÔÕÖØÚ'), 'CEEEEIIIIOOOOOUUUYY');
}
...
$name = $table_prefix . "_user_details";
$res = mysql_query("create table if not exists $name (" .
	"user int(11) not null primary key, group_id int(11) not null, " .
    "abonnement int(11), naissance date)", $mambodb);
if(!$res)
...
while ($data = fgetcsv ($handle, 1000, $csv_delimiter)) {
    ...
    $naissance = date_create_from_format('j/m/Y', mysql_escape_string($data[10]));
    ...
    mysql_query("INSERT INTO $table (name, username, email, password, requireReset) " .
    	"VALUES ('$name', '$username', '$email', '$passwd', '1')",
    ...
    # table with group, abonnement, birthday
    mysql_query("INSERT INTO " .$table_prefix. "_user_details (user, group_id, abonnement, naissance) ".
    "VALUES ('$id[0]', '$group', '', '$n')",
    	$mambodb) or die("failed to insert: " . mysql_error(). "\n");
